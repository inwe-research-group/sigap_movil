...
import 'package:miapp11/utilities/arc_banner_image.dart';
import 'pagos_page.dart';
import 'package:miapp11/pages/files_page.dart';
...
class DetallePage extends StatefulWidget {
  ...
  const DetallePage({Key? key, required this.recaudacion}) : super(key: key);
                        ...
                        children: <Widget>[
                          ...
                          Row(
                            children: <Widget>[
                              ...
                              Text(
                                  "Observación UPG : ${widget.recaudacion.observacionUpg?.trim()}",
                                  style: TextStyle(
                                      color: kPrimaryColor,
                                      fontSize: 14,
                                      letterSpacing: .3)),
  ...
  escogerFile() {
    setState(() {
      ...
      file = FilePicker.platform.pickFiles()
          as Future<File>; //FilePicker.getFile();
    });
    setStatus('');
  }
        ...
        if (snapshot.connectionState == ConnectionState.done &&
          ...
          if (extensionFile == '.jpeg' ||
            ...
            return CardFile(
              ...
              imagenFile: Image.file(File(tmpFile.path)),
            );
            /*return Flexible(
              child: Image.file(
                snapshot.data,
...
class CardFile extends StatelessWidget {
  ...
  CardFile({required this.pathFile, required this.imagenFile});
  @override
  Widget build(BuildContext context) {
    final _screenSize = MediaQuery.of(context).size;
